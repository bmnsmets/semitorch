import pytest
import torch
from semitorch import minplusmp

DEFAULT_RNG_SEED = 0
torch.manual_seed(DEFAULT_RNG_SEED)


def test_minplusmp_should_add() -> None:
    x = torch.Tensor([[5.0]])
...
def test_minplusmp_should_take_min() -> None:
...
def test_minplusmp_should_add_and_take_min() -> None:
    x = torch.Tensor([[0.0, 1.0, 2.0, 3.0, 4.0, -1.0]])
    a = torch.Tensor([
...
def test_minplusmp_should_add_and_take_min_2D() -> None:
...
def test_minplusmp_should_add_take_min_6D() -> None:
    x = torch.Tensor([
...
def test_minplusmp_should_error_on_different_devices() -> None:
    for [device1, device2] in [["cpu", "cuda"], ["cuda", "cpu"]]:
        ...
        with pytest.raises(AssertionError):
            torch.autograd.gradcheck(minplusmp, test_input, atol=1e-3, rtol=1e-1)


def test_minplusmp_should_error_on_wrong_dimensions() -> None:
    test_input = (
