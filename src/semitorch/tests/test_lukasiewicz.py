import pytest
import torch
from semitorch import lukasiewicz
...
def test_lukasiewicz_should_addmax_with_bias() -> None:
    x = torch.Tensor([[1.0]])
    a = torch.Tensor([[1.0], [0.5]])
...
def test_lukasiewicz_should_addmax_and_take_max() -> None:
    x = torch.Tensor([[0.0, 0.1, 0.2, 0.3, 0.4, 1.0]])
    a = torch.Tensor([[0.0, 0.0, 1.0, 0.8, 0.3, 0.1]])
    y = lukasiewicz(x, a)
    assert y.allclose(torch.Tensor([0.2]))


def test_lukasiewicz_should_addmax_and_take_max_with_bias() -> None:
    x = torch.Tensor([[0.0, 0.1, 0.2, 0.3, 0.4, 1.0]])
    a = torch.Tensor([[0.0, 0.0, 1.0, 0.8, 0.3, 0.1]])
    b = torch.ones(6)
    y = lukasiewicz(x, a, b)
...
def test_lukasiewicz_should_addmax_and_take_max2() -> None:
    x = torch.Tensor([[0.0, 0.1, 0.2, 0.3, 0.4, 1.0]])
    a = torch.Tensor([
...
def test_lukasiewicz_should_addmax_and_take_max2_with_bias() -> None:
    x = torch.Tensor([[0.0, 0.1, 0.2, 0.3, 0.4, 1.0]])
    a = torch.Tensor([
        [0.0, 0.0, 0.8, 0.12, 0.5, 0.36],
        [0.0, 1.0, 0.92, 0.3, 0.42, 0.41],
...
def test_lukasiewicz_should_addmax_and_take_max_2D() -> None:
...
def test_lukasiewicz_should_addmax_and_take_max_2D_with_bias() -> None:
    x = torch.Tensor([[0.1, 1.0]])
    a = torch.Tensor([[1.0, 0.5], [0.12, 0.14]])
    b = torch.ones((2, 1))
    y = lukasiewicz(x, a, b)
...
def test_lukasiewicz_should_addmax_take_max_6D() -> None:
    x = torch.Tensor([
...
def test_lukasiewicz_should_addmax_take_max_6D_with_bias() -> None:
    x = torch.Tensor([
        [0.4, 1.0, 0.1, 0.2, 0.19, 0.0],
        [0.5, 0.3, 0.0, 0.2, 0.17, 0.22],
        [0.21, 0.0, 1.0, 0.92, 0.3, 0.42],
...
def test_lukasiewicz_fw_bw() -> None:
    ...
    intermediate = (x1.unsqueeze(-2) + a1 - 1).to("cuda")
    y1 = torch.max(b1, torch.max(torch.max(torch.zeros(intermediate.shape, device="cuda"), intermediate), dim=-1)[0])
    y2 = lukasiewicz(x2, a2, b2)
...
def test_lukasiewicz_should_error_on_wrong_input_outside_01_interval() -> None:
    t1 = torch.tensor([5.0])
    t2 = torch.tensor([0.5])
    t3 = torch.tensor([0.0])
    ...
    with pytest.raises(AssertionError):
        lukasiewicz(t1, t2)
        lukasiewicz(t1, t2, t3)
    with pytest.raises(AssertionError):
        lukasiewicz(t2, t1)
        lukasiewicz(t2, t1, t3)
    with pytest.raises(AssertionError):
        lukasiewicz(t2, t3, t1)


def test_lukasiewicz_should_error_on_different_devices() -> None:
    for [device1, device2] in [["cpu", "cuda"], ["cuda", "cpu"]]:
        ...
        with pytest.raises(AssertionError):
            torch.autograd.gradcheck(lukasiewicz, test_input, atol=1e-3, rtol=1e-1)


def test_lukasiewicz_should_error_on_wrong_dimensions() -> None:
    test_input = (
