...
def viterbi_v1(x, a, bias=None):
    assert viterbi_check_tensor_values_in_01_interval(x), \
        f"all inputs x should be contained in the interval [0,1]"
    assert viterbi_check_tensor_values_in_01_interval(a), \
        f"all inputs a should be contained in the interval [0,1]"
    if bias is not None:
        assert viterbi_check_tensor_values_in_01_interval(bias), \
    ...
    y = checkpoint(viterbi_fw_kernel_v1, x, a, bias)
...
class Viterbi(torch.nn.Module):
    """
    Applies a Viterbi semiring transformation on [0,1] to the supplied data.
    """
...
def viterbi_parameters(model):
    return chain.from_iterable(
        m.parameters() for m in model.modules() if isinstance(m, Viterbi)
    )


def nonviterbi_parameters(model):
    return chain.from_iterable(
        m.parameters()
        for m in model.modules()
        if not isinstance(m, Viterbi) and list(m.children()) == []
    )


def viterbi_check_tensor_values_in_01_interval(tensor: torch.tensor) -> bool:
